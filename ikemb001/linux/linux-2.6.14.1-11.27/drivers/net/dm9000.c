...
// hcj.
unsigned char hcj_mac[32] = {0};
int use_kernel_mac = 0;
// Returns 0, or -errno.  arg is in kp->arg. 
int mac_set(const char *val, struct kernel_param *kp)
{
	unsigned int *_mac = (unsigned int *)hcj_mac;
	//printk("---%s---: %s\n", __FUNCTION__, val);
	//strncpy(hcj_mac, val, sizeof(hcj_mac));
	
	if(6 == sscanf(val, "%02x:%02x:%02x:%02x:%02x:%02x", 
			_mac, _mac + 1, _mac + 2, _mac + 3, _mac + 4, _mac + 5))
	{
		int i = 0;
		for(i = 0; i < 6; i++)
			hcj_mac[i] = (unsigned char)_mac[i];
		use_kernel_mac = 1;
	}
	//printk("mac: %02x:%02x:%02x:%02x:%02x:%02x\n", hcj_mac[0], hcj_mac[1], hcj_mac[2],
	//		hcj_mac[3], hcj_mac[4], hcj_mac[5]);

	return 0;
}
// Returns length written or -errno.  Buffer is 4k (ie. be short!) 
int mac_get(char *buffer, struct kernel_param *kp)
{
	//printk("---%s---\n", __FUNCTION__);
	return 0;
}
__module_param_call("", mac, mac_set, mac_get, hcj_mac, 0444);
