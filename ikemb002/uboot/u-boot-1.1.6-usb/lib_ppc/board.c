...
# endif
	...
	//initialize gpio30=1 to set dsp_rst=1
	volatile immap_t        *im = (volatile immap_t *)CFG_IMMR;
	volatile gpio83xx_t     *par_io =(volatile gpio83xx_t *)&im->pgio;
	u32 temp;
	//GPDIR
	temp = in_be32(&par_io->dir);
	temp &= ~(0x00010000);
	temp |= 0x00010000;
	out_be32(&par_io->dir, temp);

	//GPODR
	temp = in_be32(&par_io->odr);
	temp &= ~(0x00010000);
	out_be32(&par_io->odr, temp);

	//GPDAT
	temp = in_be32(&par_io->dat);
	temp &= ~(0x00010000);
	temp |= 0x00010000;
	out_be32(&par_io->dat, temp);
