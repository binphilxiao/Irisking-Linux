...
printf("\nbi_memstart = %u\n",bis->bi_memstart);
printf("bi_memsize = %u\n",bis->bi_memsize);
printf("bi_flashstart = %u\n",bis->bi_flashstart);
printf("bi_flashsize = %u\n",bis->bi_flashsize);
printf("bi_flashoffset = %u\n",bis->bi_flashoffset);
printf("bi_sramstart = %u\n",bis->bi_sramstart);
printf("bi_sramsize = %u\n",bis->bi_sramsize);
printf("bi_immrbar = %u\n",bis->bi_immrbar);
printf("bi_bootflags = %u\n",bis->bi_bootflags);
printf("bi_ip_addr = %u\n",bis->bi_ip_addr);
printf("bi_enetaddr = ");
for(ggg=0;ggg<6;ggg++)
        printf("%x ",bis->bi_enetaddr[ggg]);
printf("\n");
printf("bi_ethspeed = %u\n",bis->bi_ethspeed);
printf("bi_intfreq = %u\n",bis->bi_intfreq);
printf("bi_busfreq = %u\n",bis->bi_busfreq);
printf("bi_baudrate = %u\n",bis->bi_baudrate);
...
/*
printf("priv_regs = %x\n",TSEC_BASE_ADDR + index * TSEC_SIZE);
printf("priv_phyregs =%x\n",TSEC_BASE_ADDR + tsec_info[index].phyregidx * TSEC_SIZE);
*/
...
/*
printf("name = ");
for(ggg=0;ggg<NAMESIZE;ggg++)
printf(" %x ",dev->name[ggg]);
printf("\n");

printf("addr = ");
for(ggg=0;ggg<6;ggg++)
printf(" %x ",dev->enetaddr[ggg]);
printf("\n");
...
//printf("maccfg1----2 = %x\n",priv->regs->maccfg1);
	...
	&& !defined(BITBANGMII)
//printf("miiphy_register started!\n");
	miiphy_register(dev->name, tsec_miiphy_read, tsec_miiphy_write);
#endif

	/* Try to initialize PHY here, and return */
//printf("Try to initialize PHY here, and return!\n");
	return init_phy(dev);
}
...
//printf("write reg MIIMADD1 = %x\n",regbase->miimadd);
	regbase->miimcon = value;
//printf("write reg MIIMCON1 = %x\n",regbase->miimcon);
	asm("sync");
//printf("write reg MIIMADD2 after sync = %x\n",regbase->miimadd);
//printf("write reg MIIMCON2 after sync = %x\n",regbase->miimcon);
...
uint read_phy_reg(struct tsec_private *priv, uint regnum)
{
	 ...
	 * number into MIIMADD */
	regbase->miimadd = (phyid << 8) | regnum;
//printf("read reg MIIMADD1 = %x\n",regbase->miimadd);

	/* Clear the command register, and wait */
	regbase->miimcom = 0;
	asm("sync");
//printf("read reg MIIMCOM1 = %x\n",regbase->miimcom);

	/* Initiate a read command, and wait */
	regbase->miimcom = MIIM_READ_COMMAND;
	asm("sync");
//printf("read reg MIIMCOM2 = %x\n",regbase->miimcom);
...
/*
printf("phy_info id = %x\n",priv->phyinfo->id);
printf("phy_info name = %s\n",*priv->phyinfo->name);
printf("phy_info shift = %x\n",priv->phyinfo->shift);
printf("phy_info config reg = %x\n",priv->phyinfo->config->mii_reg);
printf("phy_info config data = %x\n",priv->phyinfo->config->mii_data);

printf("phy_info startup reg = %x\n",priv->phyinfo->startup->mii_reg);
printf("phy_info startup data = %x\n",priv->phyinfo->startup->mii_data);

printf("phy_info shutdown reg = %x\n",priv->phyinfo->shutdown->mii_reg);
printf("phy_info shutdown data = %x\n",priv->phyinfo->shutdown->mii_data);
*/
...
uint mii_parse_rtl8201(uint mii_reg, struct tsec_private * priv)
{
	return 0;
}
...
struct phy_info phy_info_rtl8201 = {
        0x8201,
        "Realtek RTL8201",
        6,
        (struct phy_cmd[]){     /* config */
