...
#include <linux/skbuff.h>
#include <linux/tracer.h> /* AK : tracer utility */
...
static inline int ipsec_tunnel_xmit2(struct sk_buff *skb)
{
	...
	return dst_output(skb); /* AK : called */
#else
	return ip_send(skb);
#endif
}
...
ipsec_tunnel_send(struct ipsec_xmit_state*ixs)
{
	...
	//printk(KERN_INFO "trace 1 in %s ...\n", __FUNCTION__);
	ixs->skb->dev = ixs->physdev;
#ifdef NETDEV_25
	//printk(KERN_INFO "trace 2 in %s ...\n", __FUNCTION__);
...
#else
	/*skb_orphan(ixs->skb);*/
	//printk(KERN_INFO "trace 3 in %s ...\n", __FUNCTION__); /* AK : not executed */
	...
	//printk(KERN_INFO "trace 8 in %s ...\n", __FUNCTION__); /* AK not executed */
	ip_send(ixs->skb);
#endif /* NETDEV_23 */
#else /* NET_21 */	/* 2.0 kernels */
	//printk(KERN_INFO "trace 9 in %s ...\n", __FUNCTION__); /* AK not executed */
	ixs->skb->arp = 1;
...
#endif /* NET_21 */
	//printk(KERN_INFO "trace 10 in %s ...\n", __FUNCTION__);
	...
	enum ipsec_xmit_value stat)
{
	...
	if(stat != IPSEC_XMIT_OK) {
		if(stat == IPSEC_XMIT_PASS) {
			//printk("AK : %s jumping to bypass and tunnel send \n", __FUNCTION__);
			goto bypass;
		}
		...
		KLIPS_PRINT(debug_tunnel & DB_TN_XMIT,
				...
				stat);
		//printk("AK : %s cleaning up bad status \n", __FUNCTION__);
		goto cleanup;
	}
...
ipsec_tunnel_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//printk("1,");
	//trace_set_L1(b,20); /* ipsec id */
	//trace_set_L1(d, 1); /* trace point */
	//trace_log_L1(trace_var(b), trace_var(d), 100); /* id, trace point, null */

	//trace_log_L1(100, 1,100 );
	ipsec_xsm(ixs);
	//trace_log_L1(100, 2,100 );

	//trace_set_L1(b,20); /* ipsec id */
	//trace_set_L1(d, 2); /* trace point */
	//trace_log_L1(trace_var(b), trace_var(d), 100); /* id, trace point, null */
	//printk("2 \n");
 ...
 cleanup:
	printk("AK : %s bad status, tunnel cleanup \n", __FUNCTION__);
	ipsec_tunnel_cleanup(ixs);
...
//static int ipsec_ixs_max = 100000; /* AK : 64B experiment */
//static int ipsec_ixs_max = 24; /* AK : experiment 1: works but poor performance */
//static int ipsec_ixs_max = 512; /* AK : experiment 2: keep crypto_q_max = 24 */
...
ipsec_xmit_state_new (void)
{
	...
	//printk("AK : size of ipsec_xmit_state = %d \n", sizeof(struct ipsec_xmit_state));
