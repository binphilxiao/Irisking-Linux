...
static int __init mpc83xx_spi_init(void)
{
    ...
    np = of_find_compatible_node(NULL, "spi", "mpc83xx_spi");
    if (!np)
        {
        ret = -ENODEV;
        goto err;
    ...
    dev = platform_device_register_simple("mpc83xx_spi", 0, r, 2);
    if (IS_ERR(dev))
        {
        ret = PTR_ERR(dev);
        goto err;
    ...
    ret = platform_device_add_data(dev, &spi_data, sizeof(struct fsl_spi_platform_data));
    if (ret)
        goto unreg;
