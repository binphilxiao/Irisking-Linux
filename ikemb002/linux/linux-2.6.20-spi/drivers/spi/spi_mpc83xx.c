...
int mpc83xx_spi_bufs(struct spi_device *spi, struct spi_transfer *t)
{
	...
	mpc83xx_spi_write_reg(&mpc83xx_spi->base->mode, SPMODE_INIT_MASTER);
...
printk("\nrx_data = %x\n",rx_data);
	...
	if (mpc83xx_spi->count) {
		if (mpc83xx_spi->tx) {
		/* set SPCOM[LST] before the last character */
            if (mpc83xx_spi->count == 1)
                mpc83xx_spi_write_reg(&mpc83xx_spi->base->command, 0x400000);
