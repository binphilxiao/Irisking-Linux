...
#define FSL_UDC_RESET_TIMEOUT 1000
	...
	VDBG("vir[qh_base] is %p phy[qh_base] is 0x%8x reg is 0x%8x",
	     (int)udc->ep_qh, (int)tmp, fsl_readl(&dr_regs->endpointlistaddr));
	...
	case FSL_USB2_PHY_UTMI_WIDE:
		portctrl |= (PORTSCX_PTS_UTMI | PORTSCX_PTW_16BIT);
		break;
	...
	tmp = 0x0000001e;	/* starts from 0x0, size 2G */
	__raw_writel(tmp, &usb_sys_regs->snoop1);
	tmp = 0x8000001e;	/* starts from 0x8000000, size 2G */
...
/* Setup qh structure and ep register for ep0. ep0 should be initialized
 * before bind() by gadget drivers */
 ...
 * ep0 doesn't use this routine. It is enable in probe()
-------------------------------------------------------------------------*/
			 ...
			 const struct usb_endpoint_descriptor *desc)
{
	...
	/* high speed int and iso ep */
	retval = -EINVAL;

	/* Disable automatic zlp generation.  Driver is reponsible to indicate
	 * explicitly through req->req.zero.  This is needed to enable multi-td
	 * request. */
	/* set 1 to disable automatic generation */
	zlt = 1;
...
//	.fifo_status	= fsl_ep_fifo_status,	/* report bytes in fifo */
			       ...
			       struct usb_ctrlrequest *setup)
{
	...
	{	/* status transaction */
		...
		if ((setup->bRequestType & USB_RECIP_MASK) ==
			...
			rc = fsl_ep_set_halt(&ep->ep, (setup->bRequest ==
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	fsl_udc_suspend(to_platform_device(udc_controller->gadget.dev.parent),
			state);
...
#endif
	printk(KERN_INFO "fsl_udc: %s bind to driver %s \n",
 ...
 * Return the udc handle on success or NULL on failure
 ------------------------------------------------------------------*/
static struct fsl_udc *struct_udc_setup(struct platform_device *pdev)
{
	struct fsl_udc *udc = NULL;
...
static int __init fsl_udc_probe(struct platform_device *pdev)
{
	...
	usb_sys_regs = (struct usb_sys_interface *)((u32)dr_regs +
						    USB_DR_SYS_OFFSET);
