				...
				int mode)
{
	...
	if (mode == DAC2) {
		...
		snd_es1938_mixer_write(chip, 0x70,0x6E );
...
//			       (mono ? 0 : 2) | (is8 ? 0 : 1));
	snd_es1938_mixer_write(chip, ESSSB_IREG_AUDIO2CONTROL2, 0x43);
	
	/* set DAC Mixer Volume*/
	snd_es1938_mixer_write(chip, ESSSB_IREG_AUDIO2, 0x88);
