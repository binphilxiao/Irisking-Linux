...
ipsec_xsm(struct ipsec_xmit_state *ixs)
{
	...
	while (ixs->state != IPSEC_XSM_DONE) {
		...
		if (stat == IPSEC_XMIT_OK) {
			...
			//printk("1 state=%d, next state=%d\n", ixs->state, ixs->next_state);
			//printk("%d->", ixs->state); 
			//trace_set_L1(b,20); /* ipsec id */
			//trace_set_L1(d,ixs->state);
			//trace_log_L1(trace_var(b), trace_var(d), 101); /* id, state value, state machine id */
			ixs->state = ixs->next_state;
		} else if (stat == IPSEC_XMIT_PENDING) {
			//printk("%d->", ixs->state); 
			//trace_set_L1(b,20); /* ipsec id */
			//trace_set_L1(d,ixs->state);
			//trace_log_L1(trace_var(b), trace_var(d), 102); /* id, state value, state machine id */
			/*
			 * things are on hold until we return here in the next/new state
			 * we check our SA is valid when we return
			 */
			spin_unlock(&tdb_lock);
			//printk("2 state=%d, next state=%d\n", ixs->state, ixs->next_state);
			return;
		} else {
	...
	//trace_set_L1(b,20); /* ipsec id */
	//trace_set_L1(d,4);
	//trace_log_L1(trace_var(b), trace_var(d), 100); /* id, state value, state machine id */
